...
const Enquiry = () => {
  // Refs for the email form and the hidden PDF template
  const form = useRef();
  ...
  // Redirect Logic: Counts down 25 seconds after submission then returns home
  useEffect(() => {
    let timer;
  ...
  // Sync form inputs with formData state
  ...
  const handleFileChange = (e) => {
    ...
    if (selectedFiles.length > 0) {
      setFiles((prev) => [...prev, ...selectedFiles]); // Append new files to state
      
      /**
       * CRITICAL FIX: We clear the input value. 
       * Without this, if a user removes a file and tries to add the same one again,
       * the 'onChange' event won't trigger because the filename hasn't changed.
       */
      e.target.value = ""; 
    }
  };

  // Removes a specific file from the selection list
  const removeFile = (index) => {
    setFiles(files.filter((_, i) => i !== index));
  };

  // Submits form data via EmailJS
  const handleSubmit = async (e) => {
    e.preventDefault();
    setIsSending(true);
    
    // Generate a simple 5-digit reference number
   ...
   * 1. Captures a hidden HTML element using html2canvas.
   * 2. Converts that capture into a PNG.
   * 3. Adds the PNG to a jsPDF instance for downloading.
   */
  const generatePDF = async () => {
    setIsGeneratingPDF(true);
    
    // Wait briefly for images/styles to settle in the hidden DOM
    ...
    const options = {
      ...
      windowWidth: 794, // Standard A4 pixel width at 72DPI
    };
  ...
  // Tailwind class constants for cleaner JSX
  ...
  return (
    <section className="relative py-20 md:py-32 bg-slate-50 dark:bg-slate-950 transition-colors overflow-hidden print:bg-white print:py-0">
        ...
        <AnimatePresence mode="wait">
          {submitted ? (
            ...
            >
              ...
              <p className="mb-8 text-slate-500 font-mono">Reference ID: <span className="text-blue-600 font-bold">#{refNumber}</span></p>
      ...
      {/* --- HIDDEN PDF TEMPLATE --- 
          This div is pushed far off-screen (-20000px). 
          It is only used by html2canvas to render the document for the PDF download.
      */}
      <div style={{ position: "absolute", top: "-20000px", left: "0" }}>
        <div ref={pdfExportComponent} style={{ width: "794px", minHeight: "1123px", padding: "60px", backgroundColor: "white", color: "#0f172a", fontFamily: "Arial, sans-serif", display: "flex", flexDirection: "column" }}>
          ...
          {/* PDF HEADER: Logo on left, Boxed logo on right */}
          <div style={{ 
            display: "flex", 
            ...
            <div style={{ flex: 1, display: "flex", justifyContent: "flex-end", alignItems: "center" }}>
              {/* Blue box for visibility of the secondary logo */}
              <div style={{ 
          ...
          {/* PDF GRID: Project and Client details */}
          ...
          {/* PDF DESCRIPTION: pre-wrap preserves line breaks from textarea */}
          ...
          {/* PDF FILE LIST: Shows names of selected local files */}
          {files.length > 0 && (
